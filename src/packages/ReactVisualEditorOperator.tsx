...
import {Alert, Button, CheckboxOptionType, Form, Input, InputNumber, Radio, Select} from "antd";
...
const booleanList: Array<CheckboxOptionType | string | number> = [
    {
        label: '否',
        value: 0
    },
...
function renderEditor({editData, key, config, propName, apply}: {
    ...
    switch (config.type) {
        case VisualEditorPropsType.boolean:
            return (
                <Form.Item label={config.label} name={['props', propName]} key={key}>
                    <Radio.Group options={booleanList}></Radio.Group>
                </Form.Item>
            )
